...
class EventEngine:
    ...
    def _load_stats(self):
        if os.path.exists(STATS_FILE):
            try:
                ...
                pass
        return {"total_attacks": 0, "active_bans": 0}

    def _update_stats(self):
        # Count active bans
        active_bans = 0
        for ip, data in self.rbac.roles_data.items():
            if data['role'] == 'isolated':
                active_bans += 1
        
        self.stats['active_bans'] = active_bans
    ...
    def handle_threat(self, ip, reason):
        ...
        self._update_stats() # Update ban count
